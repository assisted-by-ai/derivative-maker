...
- `derivative-maker --tag v1.2.3` names a release by tag
...
Proven by experiment on Debian trixie `sequoia-git 0.4.0-4+b5` (Dec
2025):

```
$ git tag del                             # lightweight, no -a
$ git tag del -m .                        # annotated, no -s
$ git tag -v del                          # git's own verifier
error: del: cannot verify a non-tag object of type commit.
...
error: no signature found.

$ sq-git log --trust-root HEAD del        # our verifier
...
- **Advantage over A:** none in practice. Semantically cleaner (uses
  git's own atom) but the underlying parser is the same.
...
| Honest developer with PGP-looking release notes | creates unsigned tag with spoof-triggering text | - | substring check, `%(contents:signature)`, `sq packet dump`, replay-resistant check |
