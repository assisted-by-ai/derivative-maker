...
- https://gitlab.com/sequoia-pgp/sequoia-git/-/issues
...
- **SSH signatures** (`-----BEGIN SSH SIGNATURE-----`): NOT supported
  ...
  tags fail verification naturally. No special code handling needed.
...
- It's the OpenPGP standard (RFC 4880 / draft-ietf-openpgp-crypto-refresh).
...
Both require access to a trusted key:
...
## Spoofability analysis (historical, for reference)

Before the current `sqop verify` implementation, several approaches
were tried and found insufficient:

### Attempt 1: substring check on tag body

```bash
case "$tag_body" in *'-----BEGIN PGP SIGNATURE-----'*) ...
```

**Spoofable**: an unsigned tag whose message contains the marker
strings passes the check. Trivially exploitable by putting PGP-looking
text in the tag message.

### Attempt 2: git for-each-ref %(contents:signature)

```bash
sig="$(git for-each-ref --format='%(contents:signature)' "$ref")"
```

**Also spoofable**: git's `parse_signature()` finds the first BEGIN
marker and returns everything from there to end-of-body. It does
NOT cryptographically verify. Same spoofing surface as substring.

Tested on Debian trixie -- both approaches accept a tag with fake
PGP markers in the message body.

### Attempt 3: sq packet dump (structural check)

Feed the extracted signature block to `sq packet dump` to verify
it's structurally valid OpenPGP.

**Closes spoofs 1-2** (random text fails packet parsing) but
**does NOT close replay attacks**: an attacker can paste a
structurally valid signature block copied from a previous real
signed tag. The packet structure is valid -- it just wasn't made
over THIS tag's payload.

### Attempt 4 (current): sqop verify (full cryptographic verification)

Extract trusted certs from `openpgp-policy.toml`, split tag body
into payload + signature, run `sqop verify sig cert < payload`.

**Closes all spoofing attacks** including replay: `sqop verify`
checks that the signature was made over the specific payload
(the tag text), not just that the signature is structurally valid.
...
- If the Kicksecure release workflow adds cross-signing tags with
  commit keys, the same trust store could serve both.
