...
- `derivative-update` (6 call sites: initial all-check, tag verification, submodule verification, post-merge re-verification)
- `build-steps.d/1100_sanity-tests` (1 call site: `--mode all`)
...
Sourcing `help-steps/variables` does `cd "$source_code_folder_dist"`.
This would break:
1. Submodule foreach (cwd must stay in the submodule)
2. Standalone use from a subdirectory (cwd should be preserved)

Detection: `dist_build_one_parsed` flag (set by `variables` after
running `parse-cmd`). If "true", bootstrap is skipped and inherited
env vars are used.

In the standalone path, `$PWD` is saved before sourcing and restored
after with `cd -- "$orig_pwd"`.
...
cause a hard error (cannot re-source `variables` without changing
cwd).
...
authentication using the existing `sign_tag` policy field would
